...
best_params = {
    ...
    "learning_rate": 0.012163299704553493,
    "max_depth": 7,
    "n_estimators": 1460,
    "subsample": 0.9519260362533433,
}
...
# %%
resid_idx_export = resid_idx[resid_idx['residuals'].abs()>=50]
resid_path = os.path.join(path,'outputs','model-tables','table-residuals-analysis.csv')
resid_idx_export.groupby('positive_residual').describe().T.to_csv(resid_path)
...
# %%
resid_idx["abs_error"] = np.abs(resid_idx['residuals'])
resid_idx["decile"] = pd.qcut(resid_idx["predicted"], 10, labels=False)

error_by_decile = (
    resid_idx.groupby("decile")
      .agg(
...
# %%
error_path = os.path.join(path,'outputs','model-tables','table-error-deciles.csv')
error_by_decile.to_csv(error_path)

# %% [markdown]
# ### Error by decile embeddings

# %%
xgb_embdedings_model = XGBRegressor(
    **best_params,
    objective="reg:squarederror",
    tree_method="hist",
    random_state=42
...
xgb_embdedings = ml_model(xgb_embdedings_model)
xgb_embdedings.load_data(baseline_embeddings_x_idx, X_all, y_all, train_bayes, test_bayes)
xgb_embdedings.transformation()
xgb_embdedings.train()
xgb_embdedings.predict_evaluate()

xgb_no_embdedings_model = XGBRegressor(
    **best_params,
    objective="reg:squarederror",
    tree_method="hist",
    random_state=42
...
xgb_no_embdedings = ml_model(xgb_no_embdedings_model)
xgb_no_embdedings = ml_model(xgb_tuned)
xgb_no_embdedings.load_data(baseline_x_idx, X_all, y_all, train_bayes, test_bayes)
xgb_no_embdedings.transformation()
xgb_no_embdedings.train()
xgb_no_embdedings.predict_evaluate()

# %%
# Compare groups with large residuals
resid_idx['predicted_embeddings'] = xgb_embdedings.y_pred
resid_idx['predicted_no_embeddings'] = xgb_no_embdedings.y_pred
resid_idx['residuals_embeddings'] = resid_idx['observed'] - resid_idx['predicted_embeddings']
resid_idx['residuals_no_embeddings'] = resid_idx['observed'] - resid_idx['predicted_no_embeddings']

# %%
resid_idx["abs_error_embeddings"] = np.abs(resid_idx['residuals_embeddings'])
resid_idx["abs_error_no_embeddings"] = np.abs(resid_idx['residuals_no_embeddings'])
resid_idx["decile_embeddings"] = pd.qcut(resid_idx["predicted_embeddings"], 10, labels=False)

error_by_decile_embeddings = (
    resid_idx.groupby("decile")
      .agg(
          mean_pred_embeddings=("predicted_embeddings", "mean"),
          mae_embeddings=("abs_error_embeddings", "mean"),
          rmse_embeddings=("abs_error_embeddings", lambda x: np.sqrt(np.mean(x**2))),
          count_embeddings=("abs_error_embeddings", "size"),
          mean_pred_no_embeddings=("predicted_no_embeddings", "mean"),
          mae_no_embeddings=("abs_error_no_embeddings", "mean"),
          rmse_no_embeddings=("abs_error_no_embeddings", lambda x: np.sqrt(np.mean(x**2))),
          count_no_embeddings=("abs_error_no_embeddings", "size")
      )
)
...
# %%
calib_plot = resid_idx.copy()
calib_plot["bin"] = pd.qcut(calib_plot["predicted"], 10)

calibration = (
    calib_plot.groupby("bin")
      .agg(
          mean_pred=("predicted", "mean"),
...
# %%
plt.plot(calibration["mean_pred"], calibration["residual_std"], marker="o")
plt.xlabel("Predicted EUI")
plt.ylabel("Residual standard deviation")
plt.title("Residual spread vs predicted EUI")
plt.savefig(os.path.join(path,'outputs','model-figures','figure-calibration-plot.png')) 
plt.savefig(os.path.join(path,'outputs','model-figures','figure-calibration-plot.eps'))
plt.show()
...
# %%
train_cal, val_cal, test_cal = randomise_X_y_idx(df_proc_2, split=0.8, random_seed=bayes_seed[0], val_split=0.1)

# %%
xgb_cal_model = XGBRegressor(
    **best_params,
    objective="reg:squarederror",
    tree_method="hist",
    random_state=42
...
xgb_cal = ml_model(xgb_cal_model)
xgb_cal.load_data(all_x_idx, X_all, y_all, train_cal, test_cal)
xgb_cal.transformation()
xgb_cal.train()
xgb_cal.predict_evaluate()

# %%
# Calculate scores
scores = np.abs(xgb_cal.y_test - xgb_cal.y_pred)
alpha = 0.1
q_hat = np.quantile(scores, 1 - alpha)

# %%
y_test_pred = xgb_cal.model.predict(X_all[val_cal])
y_true = y_all[val_cal]
...
print(f'Coverage is {coverage:.2f}, with mean bounds of ({lower_mean:.2f},{upper_mean:.2f})')
